...
- Fixed **Container restarts**/**Pod restarts** drawing a fake 2-hour band for a single, isolated restart - confirmed live (a pod restarted once at 17:14, marker shown as 17:14-19:14). `increase()` over a window stays positive for the *entire* window after even one restart, so `v2.2.7`'s fixed 2h window (right for surviving Thanos downsampling, wrong for shape) drew every restart the same way regardless of whether it repeated. Shrunk to 10m for these two layers only, sized off Kubernetes' own CrashLoopBackOff behavior (its backoff delay between restarts caps at 5m) - a still-looping pod always restarts again within 10m and keeps the band growing, while an isolated restart reverts to a short blip close to a line once nothing follows. Trade-off: 10m can't reliably span two of Thanos's downsampled points the way 2h could, so a single isolated restart old enough to already be downsampled risks going undetected again.
...
- Reworked the Node Drilldown's Dependencies tab layout for nodes running many pods (30-40 in practice): switched the Node Graph panel from Grafana's default force-directed physics layout to its "layered" layout, and reversed the vSphere-chain edges (vCenter → VCF cluster → ESXi host → node, rather than node → ESXi → …) so the layered ranking puts the physical infrastructure chain in columns to the node's left and every pod in columns to its right, with the node itself in the center - instead of a force-directed jumble that got unreadable well before pod counts this high. Pods within their column are also now ordered by CPU share, heaviest first.
...
- Fixed the Workload Drilldown's CPU and Memory tabs' "Pods" tables having no link from the POD column to the Pod Drilldown - the Overview tab's own Pods table already had one. The URL prefix is passed in pre-built from the route params, since this table's frame excludes cluster/namespace/workload.
- Fixed all five of the Kubernetes home page's Efficiency tab tables ("Containers without resource requests / memory limits / CPU limits", "CPU/Memory over-requested containers") having no Cluster/Namespace/Pod/Workload links either - the same gap the Overview tab's "Issue details" table had before v1.11.2.
- Fixed the "Scheduling: Containers with CPU/Memory requests set" stat panel on all six Namespace/Workload/Pod Drilldown CPU and Memory tabs showing red at 100%. It was reusing `usageThresholds`, a *utilisation* scale where over 90% means "about to hit the ceiling" - but this is a coverage ratio where 100% is the goal. Now uses a new mirrored `coverageThresholds` (under 60% red, 60-90% orange, 90%+ green).
- Added a Workload Drilldown link to the `deployment` column on the Kubernetes home page's "Issue details" table ("Zero replica deployments" view) - that query returns a bare `deployment` label with no workload/workload_type of its own, so the link inlines `deployment` as the type segment.
- Extracted the Cluster/Node/Namespace/Workload/Deployment/Pod table-link overrides into one shared `applyEntityDrilldownLinks` helper (`src/scenes/drilldownLinks.ts`), now used by the Alerts table, the Kubernetes home page's Issue details table, and the Efficiency tab's tables, instead of a fourth copy of the same override block. Also breaks the `kubernetesOverviewScene`/`kubernetesEfficiencyScene` -> `clustersApp` -> `kubernetesPage` import cycles' link-related half.
- Added a **"Copy as kubectl"** column to the Namespaces, Workloads and Nodes list tables, the Workload and Node Drilldowns' Pods tables, and the Alerts table: an icon-only first column whose menu copies ready-to-paste kubectl commands for that exact row - describe/logs/YAML/events/exec for a pod, describe/logs/YAML/list-pods/events for a workload, describe/list-pods/cordon/drain for a node, describe/get-all/quotas/events for a namespace. The `--context <cluster>` flag is a best-effort convenience: a Prometheus `cluster` label and a kubeconfig context name are independent strings that only usually agree.
- Added **rollout and container-restart markers** as annotation layers on the Workload and Pod Drilldowns, drawn across every timeseries panel on all 7 tabs with a toggle in the page controls. Rollouts come from `changes(kube_<kind>_metadata_generation[...]) > 0`, restarts from `changes(kube_pod_container_status_restarts_total[...]) > 0`, following the shape Grafana's own Prometheus-annotations documentation prescribes. Neither will ever show in the local demo stack - its metrics file is served statically, so no counter in it ever changes; same limitation as the existing `rate()`/`increase()`-based panels.
- Added an **"Investigate" button to every Drilldown page title** (Cluster, Node, Namespace, Workload, Pod), opening the Assistant with that page's own entity as context. Previously the Assistant was only reachable from the Alerts table, so it disappeared exactly when you clicked through from an alert into the entity itself. The Alerts table's own Investigate prompt now also carries node/workload, and its Action column moved to the front of the table - it used to sit permanently off-screen behind horizontal scroll.
- Added a NODE column (linked to the Node Drilldown) to the Alerts table - the page has had a Node filter all along, but the table never showed which node a node-scoped alert was about.
- Removed 17 unused exports from `src/queries/prometheus.ts` (`countClusters`, `firingAlerts`, `clusterInventory`, `clusterCpuUsage`, `podPhases`, ...) - leftovers from an early iteration, superseded by the per-page `queries/*Queries.ts` files and with no remaining consumer.
- Moved the `SectionHeading` component into its own `src/scenes/sectionHeading.tsx`. It was declared identically in six files, and the Kubernetes home page's two scenes imported it from `clustersApp.tsx`, creating an import cycle for a three-line component.
- Fixed `App.test.tsx`, which had been failing since v1.7.0: it destructured the Clusters page out of `pages[0]`, but the Kubernetes home page was inserted ahead of it. Now looks the page up by title.
...
- Added a panel menu ("..." button, previously absent entirely on every panel in this app) with a single "Explore" item to every panel app-wide: clicking it opens Grafana Explore with that exact panel's queries, datasource, and time range prefilled, using `@grafana/scenes`' own `getExploreURL`. Wired via one `$behaviors` entry per scene rather than touching each individual panel-builder call.

...
export function getCronjobMemoryScene(cluster: string, namespace: string, clusterRegex: string, namespaceRegex: string) {
  ...
  // cronjob's own pods - same reasoning as the CPU tab's own table and the
  // Overview tab's optimization panels). All 4 queries carry the same
  // (cluster, namespace, workload, workload_type, pod, join_key) label set,
  // so `merge` aligns them without ambiguity.
